...
def collect_daily_prices_batch(
    ...
    result = {stock.code: [] for stock in warning_stocks}
    ...
    for stock_info in warning_stocks:
        ...
        if is_active:
            ...
            calc_release_date = release_date
    ...
    for i, date in enumerate(target_dates, 1):
        ...
        target_codes = [
            code for code, period in stock_periods.items()
            ...
            if code in df_market.index:
                ...
                try:
                    ...
                    daily_price = DailyPriceData(
                        code=code,
                        name=stock_periods[code]['info'].name,
                        date=date,
                        close=close_price,
...
def collect_daily_prices(
    warning_stock: InvestmentWarningStock, 
    trading_days_after_release: int = 3,
    use_cache: bool = True
) -> List[DailyPriceData]:
    """
...
if __name__ == "__main__":
    ...
    test_stock = InvestmentWarningStock(
        ...
        name="웰킵스하이텍",
        market="코스닥",
        designation_date=datetime(2026, 2, 11),
        release_date=None
    )
