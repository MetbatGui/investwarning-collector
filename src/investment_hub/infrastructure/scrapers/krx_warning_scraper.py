...
def fetch_investment_warning_stocks(start_date: str = None, end_date: str = None) -> List[InvestmentWarningStock]:
    """
    ...
    name_to_code_mapping = pykrx_adapter.get_ticker_name_mapping(
        mapping_date, 
        market="ALL", 
        use_cache=True
    )
    ...
    try:
        ...
        table = None
        for t in tables:
            if 'list' in t.get('class', []) and 'type-00' in t.get('class', []):
                table = t
                break
        ...
        for row in rows:
            ...
            if market_img and market_img.has_attr('alt'):
                market = market_img['alt']
            elif market_img and market_img.has_attr('title'):
                market = market_img['title'] # Sometimes title is used?
