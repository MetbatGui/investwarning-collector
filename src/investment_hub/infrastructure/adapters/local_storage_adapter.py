...
class LocalStorageAdapter(StoragePort):
        ...
        try:
            ...
            if full_path.exists():
                try:
                    ...
                    logger.warning(f"[LocalStorage] [WARN] {full_path.name} 사용 중. {backup_path.name}로 저장.")
            else:
                tmp_path.replace(full_path)
            return True
            ...
            if tmp_path.exists():
                tmp_path.unlink()
            logger.error(f"[LocalStorage] [ERROR] Excel 저장 실패 ({path}): {e}")
            return False
    ...
    def save_dataframe_csv(self, df: pd.DataFrame, path: str, **kwargs) -> bool:
        ...
        try:
            ...
            if tmp_path.exists():
                tmp_path.unlink()
            logger.error(f"[LocalStorage] [ERROR] CSV 저장 실패 ({path}): {e}")
            return False
    ...
    def save_parquet(self, df: pd.DataFrame, path: str, **kwargs) -> bool:
        ...
        try:
            ...
            if tmp_path.exists():
                tmp_path.unlink()
            logger.error(f"[LocalStorage] [ERROR] Parquet 저장 실패 ({path}): {e}")
            return False
    ...
    def save_workbook(self, book: openpyxl.Workbook, path: str) -> bool:
        ...
        try:
            ...
            if tmp_path.exists():
                tmp_path.unlink()
            logger.error(f"[LocalStorage] [ERROR] Workbook 저장 실패 ({path}): {e}")
            return False
    ...
    def load_workbook(self, path: str) -> openpyxl.Workbook | None:
        ...
        try:
            ...
            logger.error(f"[LocalStorage] [ERROR] Workbook 로드 실패 ({path}): {e}")
            return None
    ...
    def load_dataframe(self, path: str, sheet_name: str | None = None, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[LocalStorage] [ERROR] DataFrame 로드 실패 ({path}): {e}")
            return pd.DataFrame()
    ...
    def load_parquet(self, path: str, **kwargs) -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(f"[LocalStorage] [ERROR] Parquet 로드 실패 ({path}): {e}")
            return pd.DataFrame()
    ...
    def ensure_directory(self, path: str) -> bool:
        ...
        try:
            ...
            logger.error(f"[LocalStorage] [ERROR] 디렉토리 생성 실패 ({path}): {e}")
            return False
    ...
    def get_file(self, path: str) -> bytes | None:
        ...
        try:
            ...
            logger.error(f"[LocalStorage] [ERROR] 파일 읽기 실패 ({path}): {e}")
            return None
    ...
    def put_file(self, path: str, data: bytes) -> bool:
        ...
        try:
            ...
            logger.error(f"[LocalStorage] [ERROR] 파일 쓰기 실패 ({path}): {e}")
            return False
